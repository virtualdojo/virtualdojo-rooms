...
function App() {
      ...
      if (eventId) {
        return FirestoreService.getEvent(eventId)
          ...
          .catch(() => {
            ...
            FirestoreService.isUserRegistered(eventId,userCredential.user.uid)
  ...
  if (event && user) {
    return <EditEvent {...{ eventId, user, onCloseEvent, userId, event}}></EditEvent>;
  } else if(event) {
    return (
      <div>
        <ErrorMessage errorCode={error}></ErrorMessage>
        <JoinEvent users={event.users} {...{eventId, onSelectUser, onCloseEvent, userId}}></JoinEvent>
      </div>
    );
  }
  return (
