...
const User = ({ eventId, user }) => {
    const [{ isDragging }, drag] = useDrag({
      item: { name: user.userName, type: ItemTypes.USER },
        ...
        if (item && dropResult) {
          FirestoreService.addUserToRoom(user.userId, dropResult.room.roomId, eventId)
        }
      },
      collect: (monitor) => ({
...
const Room = ({eventId, room, users}) => {
    ...
    return (
      <div ref={drop} style={{backgroundColor }}>
        <h1>{room.roomName}</h1>
        {users.map(u => <User key={u.userId} user={u} eventId={eventId}></User>)}
      </div>
    )
  }
...
function ItemList(props) {
    ...
    useEffect(() => {
        const unsubscribe = FirestoreService.streamEventItems(eventId, {
            next: querySnapshot => {
                ...
                querySnapshot.docs? querySnapshot.docs.map(docSnapshot => docSnapshot.data()) : [];
    ...
    const getUsersByRoomId =(roomId) => {
        ...
        const usersInRoom = eventRoomsUsers.filter(ru => ru.roomId === roomId).map(ru => ru.userId)
        const users = eventUsers.filter(u => usersInRoom.includes(u.userId))
        return users
    }
    const users = eventUsers
        .map((item) => <User key={item.userId} eventId={eventId} user={item}></User>);
    const rooms = eventRooms
        .map((item) => <Room key={item.roomId} room={item} eventId={eventId} users={getUsersByRoomId(item.roomId)}></Room>);
    return (
        <div>
