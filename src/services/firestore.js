...
export const createEvent = (eventName, userName, userId) => {
    return db.collection('events')
        ...
        }).then((docRef) => {addUserToEvent(userName, docRef.id, userId, true); return docRef}).then((docRef) =>  docRef)
};
...
export const addUserToEvent = (userName, eventId, userId, isMentor = false) => {
    return getEventItems(eventId)
        .then(querySnapshot => querySnapshot.docs)
        .then(eventItems => eventItems.find(eventItem => eventItem.data().userId === userId))
        .then(matchingItem => {
           // if (!matchingItem) {
                return db.collection('events')
                    .doc(eventId)
